...
class UpayGateway extends BaseGateway
{
        ...
        try {
            ...
            if ($response->successful() && ($result['status'] ?? '') === 'success') {
                return $this->formatResponse(true, [
                    ...
                    'currency' => 'BDT',
                    'status' => 'pending',
                    'message' => 'Upay payment initialized successfully',
                    'data' => [
            ...
            return $this->formatResponse(false, [
                'transaction_id' => $payload['trx_id'],
                'status' => 'failed',
                'message' => $result['message'] ?? 'Failed to initialize Upay payment',
                'data' => $result,
            ]);
        } catch (\Exception $e) {
            $this->logTransaction('init_error', '/payment/init', $data, ['error' => $e->getMessage()]);
            return $this->formatResponse(false, [
                'message' => 'Upay initialization error: ' . $e->getMessage(),
            ]);
        }
    }
    ...
    public function verify($paymentId): array
    {
        try {
            ...
            return $this->formatResponse(false, [
                'transaction_id' => $paymentId,
                'message' => 'Upay verify error: ' . $e->getMessage(),
            ]);
        }
    }
    ...
    public function refund($paymentId, $amount): array
    {
        try {
            ...
            return $this->formatResponse(false, [
                'transaction_id' => $paymentId,
                'amount' => $amount,
                'message' => 'Upay refund error: ' . $e->getMessage(),
            ]);
        }
    }
